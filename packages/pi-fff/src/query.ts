...
export function normalizePathConstraint(
  ...
  if (path.isAbsolute(trimmed)) {
    ...
    // Out-of-workspace absolute paths produce a relative result with
    // leading "../". We no longer throw — FFF simply returns empty
    // results for such paths since nothing in the index matches them.
    // This avoids a hard error that would block the agent when CWD
    // differs from the searched directory (e.g. pi default CWD=/tmp).
    trimmed = relative;
  }
