...
describe("path constraint normalization", () => {
  ...
  test("gracefully handles absolute paths outside the workspace", () => {
    // Out-of-workspace paths no longer throw; they convert to relative
    // with leading ../ and will produce empty FFF results (not indexed).
    const result = normalizePathConstraint("/tmp/other/.agents/**", cwd);
    expect(result).toBe("../other/.agents/");
